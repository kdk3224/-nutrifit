...
function renderSponsorsV14(){
 ...
 box.innerHTML=sponsorsV14.map(s=>{
   ...
   if(a) return `<button class="sponsorSlotV14 active" onclick="showSponsorV14(${s.id})"><span class="sponsorLogoV14">⭐</span><b>${String(a.name||'Patrocinador').replace(/[<>&"']/g,'')}</b><small>Patrocinador · ${sponsorRemainingHours(a,now)} h</small></button>`;
